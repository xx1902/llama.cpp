...
DEFAULT_WORKLOADS_ROOT = (
    REPO_ROOT / "examples" / "lora-base-datasets" / "output" / "real_30"
)
...
def run_cell(
    *,
    ...
    expected_requests = min(request_limit, 30)
    ...
    options["max-delta-pairs"] = 30 if strategy.get("measure_delta_pairs") else 0
        ...
        for dataset in datasets:
            for strategy in strategies:
                ...
                if required:
                    ...
                    if not completed_cell(required_dir, min(request_limit, 30)):
                        manifest = {
                            "dataset": dataset,
