...
int main() {
    ...
    if (pure_transformer_probe_only) { // 如果只做纯模型 KV probe
        ...
        // clear_lora(ctx); // 清空当前 context 上绑定的 LoRA，此分支没有 LoRA
    ...
    std::vector<lora_node_runtime> lora_nodes = make_runtime_lora_nodes();
    ...
    for (auto & node : lora_nodes) {
        ...
        if (node.adapter == nullptr) { // 如果某个 LoRA 加载失败，释放已经加载成功的 LoRA，并退出
            fprintf(stderr, "failed to load LoRA: %s\n", node.path.c_str());

            for (auto & old : lora_nodes) { // 释放之前已经成功加载的 LoRA adapter
    ...
    std::vector<request_item> requests = make_repeated_long_context_requests(n_repeats);
