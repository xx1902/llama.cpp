...
def random_cluster_apps(
    app_names: list[str],
    target_groups: int,
    max_group_size: int,
    seed: int,
) -> list[list[str]]:
    """Balanced random grouping used only as an ablation control."""
    if target_groups * max_group_size < len(app_names):
        raise ValueError("group capacity is smaller than 87 apps")
    shuffled = list(app_names)
    random.Random(seed).shuffle(shuffled)
    base_size, remainder = divmod(len(shuffled), target_groups)
    capacities = [base_size + (1 if index < remainder else 0) for index in range(target_groups)]
    groups: list[list[str]] = []
    begin = 0
    for capacity in capacities:
        groups.append(sorted(shuffled[begin : begin + capacity]))
        begin += capacity
    return groups
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--grouping-mode",
        choices=["hybrid", "random"],
        default="hybrid",
        help="Use semantic/transition affinity or a balanced random control.",
    )
...
def main() -> None:
    ...
    if args.grouping_mode == "random":
        clusters = random_cluster_apps(
            app_names, args.group_count, args.max_group_size, args.seed
        )
    else:
