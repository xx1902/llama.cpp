// simple-sys-prefix-kv-reuse.cpp
//
// 多 LoRA 公共 prefix KV 复用实验
//
// 实验目的：
// 1. 不做 naive 的“不同 LoRA 完整 KV 直接复用”。
// 2. 只复用 base model 阶段的公共 prefix KV。
// 3. 公共 prefix 之后，每个请求再绑定各自 LoRA，计算 suffix 和 decode。
// 4. 对比：
//    - baseline_no_prefix_cache: 每个请求都重复计算公共 prefix。
//    - prefix_cache_reuse: 公共 prefix 只计算一次，然后用 llama_memory_seq_cp 复制到多个 seq。
// 5. 统计：
//    - TTFT
//    - 总耗时
//    - TPS
...
//
// 输出：
// D:/ecnu_experiment/LLama.cpp/llama.cpp/examples/simple-lora-sys/output/prefix_kv_reuse_summary.csv
...
static void apply_kv_params(
        ...
        bool use_paged_kv,
        int kv_page_size) {
    // prefix KV 复用需要同一个 KV stream。
    // 否则 llama_memory_seq_cp 只支持 full copy，不支持局部 prefix copy。
...
static bool eval_tokens(
    ...
    // 当前 experimental physical paged KV 的 cpy_k/cpy_v 路径比较受限。
    // 为了避免一次 ubatch 跨物理页，分页模式下直接按单 token decode。
    const bool force_single_token = physical_page_size > 0;

    int offset = 0;

    while (offset < (int) tokens.size()) {
        int chunk = (int) tokens.size() - offset;

        if (force_single_token) {
            chunk = 1;
        } else {
            chunk = std::min(chunk, 64);
        }
        ...
        if (ret != 0) {
            fprintf(stderr,
                    "eval_tokens failed: seq=%d start_pos=%d offset=%d chunk=%d physical_page_size=%d\n",
                    seq_id,
                    start_pos,
                    offset,
                    chunk,
...
static experiment_result summarize(
    ...
    r.use_paged_kv = use_paged_kv ? 1 : 0;
    r.n_requests = n_requests;
    r.n_lora = n_lora;
    r.n_ctx = n_ctx;
    r.kv_page_size = use_paged_kv ? kv_page_size : 0;
...
static void save_results(const std::vector<experiment_result> & results) {
    ...
    const std::string path = output_dir + "/prefix_kv_reuse_summary.csv";
    std::ofstream fout(path);

    fout << "mode,use_prefix_reuse,use_paged_kv,n_requests,n_lora,n_ctx,kv_page_size,"
...
static experiment_result run_baseline_no_prefix_cache(
    ...
    ctx_params.n_batch = use_paged_kv ? 1 : 256;
    ctx_params.n_ubatch = use_paged_kv ? 1 : 64;
...
int main() {
    ...
    fprintf(stderr, "prefix KV reuse experiment finished.\n");
