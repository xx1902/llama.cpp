...
struct sample_result {
    ...
    int suffix_delta_materialized = 0;
    double delta_materialize_ms = 0.0;
    double delta_fp32_mb = 0.0;
    double materialized_kv_mb = 0.0;
...
static sample_result run_online_suffix_delta_materialize_request(
        ...
        const std::vector<lora_node_runtime> & lora_nodes,
        const std::vector<lora_group_runtime> & groups,
        const online_prefix_node & anchor_node,
        const request_item & req,
        const request_tokens & toks,
        llama_seq_id child_full_seq_id,
        llama_seq_id request_seq_id,
        int matched_prefix_len,
        int n_predict,
        double kv_mb_per_token) {
    ...
    r.mode = "online_suffix_delta_materialize";
    r.group_name = group ? group->group_name : "unknown";
    r.lora_name = lora_nodes[req.leaf_lora_id].short_name;
    r.leaf_lora_id = req.leaf_lora_id;
    ...
    r.online_node_id = anchor_node.node_id;
    r.exact_prefix_hit = 1;
    ...
    // suffix KV 已经由 delta materialize 写回，所以这里不再 eval suffix。
    r.suffix_ms = 0.0;
...

    {
        ...
        for (int i = 0; i < (int) requests.size(); i++) {
            ...
            if (route.exact_prefix_hit &&
                ...
                // request_seq_id：刚刚正常算完 suffix 的完整 child KV
                if (route.suffix_len > 0) {
                    const llama_seq_id delta_request_seq_id =
                            (llama_seq_id) (delta_request_seq_base + i);
