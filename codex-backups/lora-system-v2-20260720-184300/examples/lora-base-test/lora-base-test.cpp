        ...
        for (const auto & variant : node.variants) {
            if (&variant == victim) {
                record_family_cache_event(
                        "tier3_delta_demote", node, victim->lora_id,
...
static bool offload_gpu_variant_to_host(
    ...
    record_family_cache_event(
            "gpu_full_to_host", *victim_node, victim->lora_id,
            request_index, options);
...
static bool ensure_cache_capacity_v2(
    ...
    while ((requires_free_sequence && free_sequences.empty()) ||
        ...
        if (victim_index < 0) return false;
        record_family_cache_event(
                "family_evict", nodes[victim_index], -1,
                request_index, options);
...
static void save_family_cache_events(const std::string & output_dir) {
    ...
    output << "request_index,action,node_id,variant_lora_id,group_name,context_id,"
