...
// 量化相关
bool llama_kv_seq_delta_materialize_branch(
        llama_context * ctx,
        llama_seq_id seq_anchor,
    ...
    llama_kv_cache::kv_delta_materialize_stats inner;
    const bool ok = kv->seq_delta_materialize_branch(
            seq_anchor,
            seq_child_delta,
            seq_dst,
            p0,
    ...
    if (!ok) {
        llama_kv_delta_materialize_set_text(
                stats->status,
                sizeof(stats->status),
                "materialize_branch_failed");
        return false;
    }

    stats->n_layers = inner.n_layers;
    stats->n_tokens = inner.n_tokens;
    stats->materialized_layers = inner.materialized_layers;
    stats->materialized_tokens = inner.materialized_tokens;
    stats->missing_cell_count = inner.missing_cell_count;
    stats->delta_fp32_bytes = inner.delta_fp32_bytes;
    stats->materialized_kv_bytes = inner.materialized_kv_bytes;
    llama_kv_delta_materialize_set_text(
            stats->status,
            sizeof(stats->status),
            "ok");
...
bool llama_kv_seq_delta_remove_branch(
        llama_context * ctx,
        llama_seq_id seq_child_delta) {
    if (ctx == nullptr) {
        return false;
    ...
    llama_memory_t mem = ctx->get_memory();
    if (auto * pure = dynamic_cast<llama_kv_cache *>(mem)) {
        return pure->seq_delta_remove_branch(seq_child_delta);
    }
    if (auto * hybrid = dynamic_cast<llama_memory_hybrid *>(mem)) {
        auto * kv = hybrid->get_mem_attn();
        return kv != nullptr && kv->seq_delta_remove_branch(seq_child_delta);
    }
    return false;
}

bool llama_kv_seq_delta_save_branch(
        const llama_context * ctx,
        llama_seq_id seq_child_delta,
        const char * path) {
    if (ctx == nullptr || path == nullptr) return false;
    llama_memory_t mem = ctx->get_memory();
    if (auto * pure = dynamic_cast<llama_kv_cache *>(mem)) {
        return pure->seq_delta_save_branch(seq_child_delta, path);
    }
    if (auto * hybrid = dynamic_cast<llama_memory_hybrid *>(mem)) {
        auto * kv = hybrid->get_mem_attn();
        return kv != nullptr && kv->seq_delta_save_branch(seq_child_delta, path);
    }
    return false;
}

bool llama_kv_seq_delta_load_branch(
        llama_context * ctx,
        llama_seq_id seq_anchor,
        llama_seq_id seq_child_delta,
        const char * path,
    ...
    llama_memory_t mem = ctx->get_memory();
    if (auto * pure = dynamic_cast<llama_kv_cache *>(mem)) {
        return pure->seq_delta_load_branch(
                seq_anchor, seq_child_delta, path, parent_node_id, child_node_id);
    }
    if (auto * hybrid = dynamic_cast<llama_memory_hybrid *>(mem)) {
        auto * kv = hybrid->get_mem_attn();
        return kv != nullptr && kv->seq_delta_load_branch(
                seq_anchor, seq_child_delta, path, parent_node_id, child_node_id);
    }
    return false;
}

bool llama_kv_seq_delta_get_branch_stats(
        const llama_context * ctx,
        llama_seq_id seq_child_delta,
        llama_kv_delta_branch_stats * stats) {
    if (ctx == nullptr || stats == nullptr) return false;
    memset(stats, 0, sizeof(*stats));
    llama_memory_t mem = ctx->get_memory();
    llama_kv_cache * kv = nullptr;
    if (auto * pure = dynamic_cast<llama_kv_cache *>(mem)) {
    ...
    llama_kv_cache::kv_delta_branch branch;
    if (!kv->seq_delta_get_branch_stats(seq_child_delta, branch)) return false;
    stats->status = 1;
    ...
    strncpy(stats->message, "ok", sizeof(stats->message) - 1);
    return true;
}

bool llama_kv_seq_delta_build_branch(
        llama_context * ctx,
        llama_seq_id seq_anchor,
        llama_seq_id seq_child_full,
        llama_seq_id seq_child_delta,
    ...
    if (ok) {
        ...
        if (branch != nullptr) {
            stats->full_kv_bytes_equivalent =
                    branch->full_kv_bytes_equivalent;

            stats->delta_q8_bytes =
                    branch->delta_q8_bytes;

            stats->delta_scale_bytes =
                    branch->delta_scale_bytes;

            const uint64_t delta_total =
                    branch->delta_q8_bytes + branch->delta_scale_bytes;

            stats->logical_saved_bytes =
                    branch->full_kv_bytes_equivalent > delta_total ?
                    branch->full_kv_bytes_equivalent - delta_total :
                    0;

            stats->logical_saved_rate =
                    branch->full_kv_bytes_equivalent > 0 ?
